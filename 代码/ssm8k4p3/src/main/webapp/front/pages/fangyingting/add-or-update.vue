	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`fangyingting`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																if((!this.ruleForm.fangyingting)){
					this.$utils.msg(`放映厅不能为空`);
					return
				}
																																								if(this.ruleForm.id){
					await this.$api.update(`fangyingting`, this.ruleForm);
				}else{
					await this.$api.add(`fangyingting`, this.ruleForm);
				}
