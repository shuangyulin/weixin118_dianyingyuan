<template>
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
					...
					<view class="lable" :style='{"width":"auto","fontSize":"28rpx","color":"#333","textAlign":"left"}'>放映厅：</view>
					<view class="text" :style='{"color":"#333","textAlign":"left","fontSize":"28rpx"}'>{{detail.fangyingting}}</view>
				</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','fangyingting');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','fangyingting');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('fangyingting', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussfangyingting/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('fangyingting', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('fangyingting', this.detail);
				this.$utils.msg('审核成功');
