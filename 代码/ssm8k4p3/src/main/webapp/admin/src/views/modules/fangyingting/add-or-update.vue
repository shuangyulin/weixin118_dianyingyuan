...
export default {
  data() {
	...
	fangyingting : false,
      ...
      rules: {
          fangyingting: [
                { required: true, message: '放映厅不能为空', trigger: 'blur' },
          ],
      }
    };
  },
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `fangyingting/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.fangyingtingCrossAddOrUpdateFlag = false;
                  this.parent.search();
